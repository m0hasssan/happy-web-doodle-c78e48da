-- Fix double-counting bug in processing section returns.
-- Previously the staging step added v_count to total_count, then the movement trigger
-- deducted it again, leaving +count residual per work order.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Stage the target row (weight only). The movement trigger will deduct weight,
    -- and for processing outflow it does GREATEST(0, total_count - NEW.count) so
    -- the count is consumed from existing inflow. Adding count here would double-count.
...
-- Backfill: recompute total_count for processing section inventory rows from movements
WITH processing_sections AS (
  SELECT id FROM public.manufacturing_sections WHERE kind = 'processing'
),
net_counts AS (
  SELECT
    si.id AS inv_id,
    ...
    si.karat,
    si.category_id,
    COALESCE(SUM(CASE WHEN m.to_type='section' AND m.to_id=si.section_id AND m.count IS NOT NULL THEN m.count ELSE 0 END), 0)
    - COALESCE(SUM(CASE WHEN m.from_type='section' AND m.from_id=si.section_id AND m.count IS NOT NULL THEN m.count ELSE 0 END), 0)
      AS net_count
  FROM public.section_inventory si
  JOIN processing_sections ps ON ps.id = si.section_id
  LEFT JOIN public.movements m ON m.metal_id = si.metal_id
    AND m.karat IS NOT DISTINCT FROM si.karat
    AND m.category_id IS NOT DISTINCT FROM si.category_id
    AND ((m.to_type='section' AND m.to_id=si.section_id) OR (m.from_type='section' AND m.from_id=si.section_id))
  GROUP BY si.id, si.section_id, si.metal_id, si.karat, si.category_id
)
UPDATE public.section_inventory si
SET total_count = CASE WHEN nc.net_count > 0 THEN nc.net_count ELSE NULL END,
    updated_at = now()
FROM net_counts nc
WHERE nc.inv_id = si.id;
