...
BEGIN
  ...
  IF NEW.from_type = 'vault' THEN
    ...
    IF NEW.count IS NOT NULL THEN
      ...
      END IF;
      -- Single-piece rule: if only one physical piece exists, must take entire weight
      -- Skip when weight=0 (count-only adjustment)
...
-- Allow movements with from_type='adjustment' or to_type='adjustment' for count-only adjustments
DROP POLICY IF EXISTS "Insert movements with permission" ON public.movements;
CREATE POLICY "Insert movements with permission" ON public.movements
FOR INSERT TO authenticated
WITH CHECK (
  (((from_type = 'vault') AND has_permission(auth.uid(), 'create_vault_entry'::app_permission, from_id))
   OR ((to_type = 'vault') AND has_permission(auth.uid(), 'create_vault_entry'::app_permission, to_id))
   OR ((from_type = 'section') AND has_permission(auth.uid(), 'access_section'::app_permission, from_id))
   OR ((to_type = 'section') AND has_permission(auth.uid(), 'access_section'::app_permission, to_id))
