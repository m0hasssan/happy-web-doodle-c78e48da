...
BEGIN
  ...
  -- Default to TRUE when no section_settings row exists, matching the table
  -- default and the apply_movement_inventory trigger behavior. Otherwise the
  -- function falls into a strict karat-match path that fails whenever the
  -- inventory was consumed via pure-equivalent (cross-karat) conversion.
  ...
  IF v_kind = 'processing' OR v_allow_karat_change THEN
    ...
    LOOP
      ...
      IF v_pure_missing <= 0.0001 AND NOT EXISTS(
      ...
      IF v_record_pure <= 0.0001 THEN CONTINUE; END IF;
