-- Track count even for processing sections (display purposes), and enforce
-- "single piece" rule: when only 1 piece exists, you must take its full weight.
...
BEGIN
  ...
  IF NEW.from_type = 'vault' THEN
    ...
    IF NEW.count IS NOT NULL THEN
      ...
      IF v_existing_count = 1 AND ABS(v_existing - NEW.weight) > 0.0001 THEN
  ...
  IF NEW.from_type = 'section' THEN
    ...
    END IF;
    IF v_from_processing THEN
      -- Processing: flexible count. Decrement count when possible, never go below zero.
      IF NEW.count IS NOT NULL AND v_existing_count IS NOT NULL THEN
        UPDATE public.section_inventory
          SET total_weight=total_weight-NEW.weight,
              total_count = GREATEST(0, total_count - NEW.count),
              updated_at=now()
          WHERE id=v_id;
      ELSE
        UPDATE public.section_inventory SET total_weight=total_weight-NEW.weight, updated_at=now() WHERE id=v_id;
      END IF;
    ELSIF NEW.count IS NOT NULL THEN
      IF COALESCE(v_existing_count,0) < NEW.count THEN
