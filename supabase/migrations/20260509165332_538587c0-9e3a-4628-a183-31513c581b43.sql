...
BEGIN
  ...
  -- Any counted category row that reached count=0 must not keep weight.
  -- Convert its remaining pure value to section shrinkage (999) before normal shrinkage math.
  FOR r IN
  ...
  LOOP
    ...
    IF FOUND THEN
      UPDATE public.section_inventory
        SET total_weight = total_weight + v_zero_count_pure,
            updated_at = now()
        WHERE id = v_loss_inv_id;
    ELSE
