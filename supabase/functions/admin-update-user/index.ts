// Edge function: update a user's username and full name (manage_users permission)
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.57.4";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type",
  "Access-Control-Allow-Methods": "POST, OPTIONS",
};

interface Payload {
  user_id: string;
  username?: string;
  full_name?: string;
}

Deno.serve(async (req) => {
  if (req.method === "OPTIONS") {
    return new Response(null, { headers: corsHeaders });
  }

  try {
    const SUPABASE_URL = Deno.env.get("SUPABASE_URL")!;
    const SERVICE_KEY = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
    const ANON_KEY = Deno.env.get("SUPABASE_PUBLISHABLE_KEY") ??
      Deno.env.get("SUPABASE_ANON_KEY")!;

    const authHeader = req.headers.get("Authorization");
    if (!authHeader) return json({ error: "Unauthorized" }, 401);

    const userClient = createClient(SUPABASE_URL, ANON_KEY, {
      global: { headers: { Authorization: authHeader } },
    });
    const { data: userData, error: userErr } = await userClient.auth.getUser();
    if (userErr || !userData.user) return json({ error: "Unauthorized" }, 401);
    const callerId = userData.user.id;

    const admin = createClient(SUPABASE_URL, SERVICE_KEY);

    const { data: canManage, error: permErr } = await admin.rpc(
      "has_permission",
      { _user_id: callerId, _permission: "manage_users" },
    );
    if (permErr) throw permErr;
    if (!canManage) {
      return json({ error: "Forbidden: missing manage_users permission" }, 403);
    }

    const body: Payload = await req.json();
    const targetId = (body.user_id ?? "").trim();
    if (!targetId) return json({ error: "user_id مطلوب" }, 400);

    const username = body.username?.trim().toLowerCase();
    const fullName = body.full_name?.trim();

    const updates: { email?: string; user_metadata?: Record<string, unknown> } = {};
    let newEmail: string | undefined;

    if (username !== undefined) {
      if (!/^[a-z0-9_.-]{2,30}$/.test(username)) {
        return json({ error: "اسم مستخدم غير صالح" }, 400);
      }
      newEmail = `${username}@users.local`;
      updates.email = newEmail;
    }
    if (fullName !== undefined) {
      updates.user_metadata = { full_name: fullName };
    }

    if (Object.keys(updates).length === 0) {
      return json({ error: "لا توجد تغييرات" }, 400);
    }

    const { error: updErr } = await admin.auth.admin.updateUserById(
      targetId,
      { ...updates, email_confirm: true } as Parameters<typeof admin.auth.admin.updateUserById>[1],
    );
    if (updErr) {
      const msg = /already|exists|duplicate/i.test(updErr.message)
        ? "اسم المستخدم مستخدم بالفعل"
        : updErr.message;
      return json({ error: msg }, 400);
    }

    const profileUpdate: Record<string, unknown> = {};
    if (newEmail) profileUpdate.email = newEmail;
    if (fullName !== undefined) profileUpdate.full_name = fullName;
    if (Object.keys(profileUpdate).length) {
      await admin.from("profiles").update(profileUpdate).eq("id", targetId);
    }

    return json({ success: true });
  } catch (e) {
    const msg = e instanceof Error ? e.message : "خطأ غير متوقع";
    return json({ error: msg }, 500);
  }
});

function json(payload: unknown, status = 200) {
  return new Response(JSON.stringify(payload), {
    status,
    headers: { ...corsHeaders, "Content-Type": "application/json" },
  });
}